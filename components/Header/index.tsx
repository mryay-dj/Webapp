...
const NAV_TABS = [
  ...
  { label: "Time Chart", id: "time-chart" },
  { label: "Anomalies", id: "anomalies" },
  { label: "Cumulative People", id: "cumulative-people" },
  { label: "Visitor Analytics", id: "visitor-analytics" },
];
...
const Header = (props: {
  ...
  const handleTabClick = (id: string) => {
    // If not on dashboard navigate there first then scroll
          ...
          <div className="flex items-center gap-2 border border-gray===-300 dark:border-strokedark p-2 rounded-xl">
            {NAV_TABS.map((tab) => (
              <button
                ...
                className="px-3 py-1.5 rounded-full text-xs font-medium bg-primary text-white hover:bg-opacity-90 transition"
              >
                {tab.label}
              </button>
            ))}
